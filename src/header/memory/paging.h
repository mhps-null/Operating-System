...
extern __attribute__((aligned(0x1000))) struct PageDirectory _paging_kernel_page_directory;
...
{
    uint8_t present_bit : 1;         // 0
    uint8_t write_bit : 1;           // 1
    uint8_t user_supervisor_bit : 1; // 2
    uint8_t write_through : 1;       // 3
    uint8_t cache_disable : 1;       // 4
    uint8_t accessed_bit : 1;        // 5
    uint8_t dirty_bit : 1;           // 6
    uint8_t use_pagesize_4_mb : 1;   // 7
...
{
    struct PageDirectoryEntryFlag flag;    // 7
    uint16_t global_page : 1;              // 8
    uint16_t ignored_1 : 3;                // 11
    uint16_t page_attribute_table_bit : 1; // 12
    uint16_t reserved_1 : 9;               // 21
    uint16_t lower_address : 10;           // 31
} __attribute__((packed));
